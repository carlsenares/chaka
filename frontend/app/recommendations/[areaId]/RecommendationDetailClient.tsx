  ...
  if (!area) {
    return (
      <main className="min-h-screen bg-base px-5 py-8 text-fg sm:px-8">
        <div className="mx-auto max-w-3xl rounded-lg border border-[#dfe7dc] bg-surface p-6 shadow-[0_1px_4px_rgb(34_54_42_/_0.05)]">
          <p className="text-sm font-semibold text-accent">Recommendation not found</p>
          ...
          <Link
            ...
            className="mt-6 inline-flex rounded-full bg-[#236b44] px-5 py-3 text-sm font-semibold text-white"
          >
            Back to Dashboard
          </Link>
        </div>
  ...
  return (
    <main className="min-h-screen bg-base px-5 py-6 text-fg sm:px-8">
      <div className="grid w-full gap-5">
        <header className="flex flex-col gap-4 border-b border-[#dfe7dc] pb-5 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <Link
              href={`/?source=${dataSourceMode}`}
              className="rounded-full border border-[#b9cabb] bg-white px-4 py-2.5 text-sm font-semibold text-fg transition hover:bg-[#edf5ee]"
            >
              Back to Dashboard
            </Link>
            {previousArea && (
            ...
            {nextArea && (
              <Link
                ...
                className="rounded-full bg-[#236b44] px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-[#1d5838]"
              >
                Next
              </Link>
            )}
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_420px]">
          <div className="grid gap-5">
            <section className="rounded-lg border border-[#dfe7dc] bg-surface p-5 shadow-[0_1px_4px_rgb(34_54_42_/_0.05)]">
              <div className="grid gap-4 sm:grid-cols-[180px_1fr]">
                ...
                <div>
                  <p className="text-sm font-semibold text-accent">AI-generated brief</p>
              ...
              <div className="mt-5 grid gap-3">
                {rationale.map((item, index) => (
                  <div key={item} className="flex gap-4 rounded-md border border-[#dfe7dc] bg-[#f8faf7] p-4">
                    <span className="flex size-7 shrink-0 items-center justify-center rounded-full bg-[#edf5ee] text-sm font-semibold text-[#236b44]">
                      {index + 1}
                    </span>
            ...
            <section className="rounded-lg border border-[#d9e4d8] bg-[#f2f7f1] p-5">
  ...
  return (
    <div>
      ...
      <div className="h-2 overflow-hidden rounded-full bg-[#e5ebe3]">
        <div className="h-full rounded-full bg-[#236b44]" style={{ width: `${Math.max(0, Math.min(value, 100))}%` }} />
      </div>
    </div>
  );
}
  ...
  return (
    <div className={heading ? "mt-5" : "mt-4"}>
      ...
      <ul className="mt-3 grid gap-2 text-sm leading-6 text-muted">
        {items.map((item) => (
          <li key={item} className="rounded-md border border-[#dfe7dc] bg-[#f8faf7] px-3 py-2">
            {item}
          </li>
        ))}
      </ul>
