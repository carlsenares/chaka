...
export function EthiopiaPriorityMap({
  ...
  useEffect(
    () => {
      if (!focusSelected || !effectiveSelectedMapAreaId || !selectedMapFeature) return;
      flyToFeature(selectedMapFeature);
    },
    [effectiveSelectedMapAreaId, focusSelected, selectedMapFeature?.properties.join_pcode],
  );
  ...
  }, [admin0Boundary, effectiveStatus, mode, showNeighborLabels]);
  ...
  return (
    <section
      className={
        frameless
          ? `overflow-hidden bg-base ${className}`
          : `overflow-hidden rounded-lg border border-[#d9d0bd] bg-[#fffdf7] shadow-sm ${className}`
      }
    >
      {!frameless && showHeader && (
      ...
      <div className={`grid ${frameless ? "min-h-[calc(100vh-5rem)]" : "min-h-[600px]"} ${showDetailsPanel ? "lg:grid-cols-[1fr_360px]" : ""}`}>
        <div className={`relative overflow-hidden ${frameless ? "min-h-[calc(100vh-9.5rem)] bg-base" : "min-h-[500px] bg-[#edf2e7]"}`}>
          ...
          {showBasemap && (frameless || !showHeader) && (
            <div className="absolute left-16 top-4 z-[500]">
              <MapModeToggle mode={mode} onChange={setMode} />
            </div>
          )}

          {!frameless && effectiveStatus === "ready" && joinedBoundaries.features.length > 0 && (
            <PriorityLegend />
          )}
